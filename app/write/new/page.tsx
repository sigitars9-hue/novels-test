...
function EditorToolbar({ exec }: { exec: (cmd: string, val?: string) => void }) {
  return (
    <div className="flex flex-wrap items-center gap-1 rounded-xl border border-white/10 bg-zinc-900/70 p-1">
      <button onClick={() => exec("bold")} className="rounded-lg px-2 py-1 hover:bg-white/10" title="Bold">
        <Bold className="h-4 w-4" />
      </button>
      <button onClick={() => exec("italic")} className="rounded-lg px-2 py-1 hover:bg-white/10" title="Italic">
        <Italic className="h-4 w-4" />
      </button>
      <button onClick={() => exec("underline")} className="rounded-lg px-2 py-1 hover:bg-white/10" title="Underline">
        <Underline className="h-4 w-4" />
      </button>
      <span className="mx-1 h-5 w-px bg-white/10" />
      <button onClick={() => exec("formatBlock", "<h2>")} className="rounded-lg px-2 py-1 hover:bg-white/10" title="Heading">
        <Type className="h-4 w-4" />
      </button>
      <button onClick={() => exec("insertUnorderedList")} className="rounded-lg px-2 py-1 hover:bg-white/10" title="Bullet List">
        <List className="h-4 w-4" />
      </button>
      <button onClick={() => exec("insertOrderedList")} className="rounded-lg px-2 py-1 hover:bg-white/10" title="Numbered List">
...
export default function WriteChapterPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // optional: suggest next chapter number
